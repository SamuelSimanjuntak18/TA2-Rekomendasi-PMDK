...
def predict():
    try:
        ...
        for i in range(len(predictions)):
            if predictions[i] == 1:
                result_df.loc[i, 'Pilihan 1'] = f"Lulus"
                result_df.loc[i, 'Pilihan 2'] = f"Tidak Lulus"
                result_df.loc[i, 'Pilihan 3'] = f"Tidak Lulus"
            else:
                result_df.loc[i, 'Pilihan 1'] = f"Tidak Lulus"
                result_df.loc[i, 'Pilihan 2'] = f"Tidak Lulus"
                result_df.loc[i, 'Pilihan 3'] = f"Tidak Lulus"
        # (Prob: {probabilities[i][0]:.10f})
        if not show_nama:
